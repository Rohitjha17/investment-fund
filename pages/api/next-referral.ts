...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    for (const referred of referredMembers) {
      const referredId = typeof referred.id === 'number' ? referred.id : parseInt(referred.id);
      const fullReferred = await db.getMember(referredId);
      if (!fullReferred) continue;

      const deposits = ((fullReferred as any).deposits || []).map((d: any) => ({
        amount: d.amount,
        date: d.deposit_date,
        percentage: d.percentage !== null && d.percentage !== undefined 
          ? d.percentage 
          : (fullReferred as any).percentage_of_return
      }));
      
      const withdrawals = ((fullReferred as any).withdrawals || []).map((w: any) => ({
        amount: w.amount,
      ...
      const referralPercent = (fullReferred as any).referral_percent || 0;
      const referralIncome = (interestEarned * referralPercent) / 100;

      totalReferralIncome += referralIncome;

      referralBreakdown.push({
        member_id: (fullReferred as any).id,
        member_name: (fullReferred as any).name,
